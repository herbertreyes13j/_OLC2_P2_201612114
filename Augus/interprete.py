...
def procesar_definicion_arr(asig,ts):
    ...
    if simbolo is None :        
        ...
        if indice not in simbolo.referencia:
            simbolo.referencia.append(indice)
        ts.actualizar(tipo_var.id,TIPO_DATO.ARREGLO,{})
...
def procesar_asign_arr(asig_arr_st,ts):
    ...
    #Arreglos y Structs
        ...
        if indice not in simbolo.referencia:
            simbolo.referencia.append(indice)
        ts.actualizar(tipo_var.id,TIPO_DATO.ARREGLO,diccionario,dim)
...
def cambiar_caracter_cadena(id,cadena,index,new_value,ts):
    if isinstance(index,int) : 
        ...
        if isinstance(new_value,int) or isinstance(new_value,float) :
            ...
            ts.actualizar(id,TIPO_DATO.CADENA,cad)
        else:
...
def procesar_accesoarray(acceso_array,ts) :
    ...
    if simbolo.tipo==TIPO_DATO.CADENA:
        if len(parametros)==1:
            try:
            ...
            LisErr.agregar(newErr)
            return None
    elif simbolo.tipo==TIPO_DATO.ARREGLO:
        temp=''
        last=''
        for par in range(0,len(parametros)):
            if(par==len(parametros)-1) :
...
def procesar_casteo(instr,ts):
    ...
    if instr.tipo_dato.value==TIPO_DATO.ENTERO.value:
    ...
    elif instr.tipo_dato.value==TIPO_DATO.FLOTANTE.value:
        if isinstance(val,int) :
            ...
            if len(val)>=1:
                ...
                LisErr.agregar(newErr)
    elif instr.tipo_dato.value == TIPO_DATO.CHAR.value:
        if isinstance(val,int) or isinstance(val,float):
            val_temp=int(val)
            if val_temp<=255:
